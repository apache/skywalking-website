...
Read [Become official Apache SkyWalking Committer](../asf/committer) to get details.

## For code developer
For developers, first step, read [Compiling Guide](../How-to-build). It teaches developer how to build the project in local and set up the environment.
...
When the E2E test is executed locally, if any test case fails, the [E2E local remote debugging function](../E2E-local-remote-debug) can be used to quickly troubleshoot the bug.

### Project Extensions
SkyWalking project supports many ways to extend existing features. If you are interesting in these ways,
read the following guides.

- [Java agent plugin development guide](../Java-Plugin-Development-Guide).
...
- If you want to build a new probe or plugin in any language, please read [Component library definition and extension](../Component-library-settings) document.
- [Storage extension development guide](../storage-extention). Help potential contributors to build a new 
...
[Observability Analysis Language Introduction](../../concepts-and-designs/oal) if you need to learn about OAL script.
- [Source and scope extension for new metrics](../source-extension). If you want to analysis a new metrics, which SkyWalking
haven't provide. You need to 
add a new receiver rather than choosing [existed receiver](../../setup/backend/backend-receivers).
At that moment, 
...
- [Thread dump merging mechanism](../backend-profile) introduces the merging mechanism, which helps the end users to understand the profile report.
- [Exporter tool of profile raw data](../backend-profile-export) introduces when the visualization doesn't work well through the official UI, how to package the original profile data, which helps the users report the issue.

## For release
[Apache Release Guide](../How-to-release) introduces to the committer team about doing official Apache version release, to avoid 
