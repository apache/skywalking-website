...
date: 2026-06-21
author: 吴晟
description: "Horizon UI 系列第四篇：Deployment 标签页如何把拓扑视角收进单个集群服务内部，并以 BanyanDB 为例说明 SkyWalking 如何观测自己的存储引擎。"
tags:
  - Cloud Native
  - Storage
---
...
- **布局是分层的。** 每个 cluster box 会按调用深度摆放 pod：source 在左，被调用对象在右，所以 upstream→downstream 链条从左到右就能看清；拖动任意 pod 后，它所属的 box 会重新流式布局，保持内容都被包住。
