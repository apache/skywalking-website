...
- **Mount** `.map` 文件到服务端 **source-map 目录**（容器镜像中是 `/app/sourcemaps`，可通过 `HORIZON_SOURCEMAPS_DIR` 指定）。这些文件在启动时按 Source Map v3 校验，按需从磁盘读取（所以不占内存预算），重启后仍然存在，会自动 reload，并且 **不能从 UI 删除**。这是生产路径：把构建产物里的 maps 放进镜像或挂载目录，它们就一直可用。
...
下一篇英文系列将进入 **Profiling**：五种 profiler（trace、async、eBPF、Go pprof、network）通过同一个火焰图呈现。
