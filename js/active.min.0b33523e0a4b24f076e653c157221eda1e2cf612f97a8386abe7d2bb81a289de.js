...
function bindClick(){$('.container a').not('.link-type').on('click',function(){var hash=$(this).attr('href')
if(hash&&/^#/.test(hash)){scrollTop(hash);$('.card-wrapper .card').removeClass('active');$(hash).parents('.card').addClass('active');}});$('.link-type').on('click',function(){var hash=$(this).attr('href')
scrollTop(hash,120);})}
