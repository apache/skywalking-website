...
---\n` + codeTxt;
    codeTxt = codeTxt
      .replace(/(\[[\s\S]*?\])\(([\s\S]*?)\)/g, function (match, p1, p2) {
        ...
        const str = p2.toLowerCase().replace(/\.md/g, "");
  ...
  for (const data of result) {
    for (const item of data.list) {
      ...
      for (const doc of item.docs) {
        ...
        const menuFileName = `${docName.replace(
          /\-|\./g,
          "_"
        )}${version.replace(/\-|v|\./g, "_")}`;
...
function handleMenuPath(list, localPath) {
  list.forEach((item) => {
    ...
    if (pagePath) {
      item.path = pagePath.startsWith("http")
        ? pagePath
        : (localPath + pagePath).toLowerCase();
    }
    if (item.catalog) {
